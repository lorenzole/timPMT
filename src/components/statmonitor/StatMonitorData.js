...
const StatMonitorData = () => {
  ...
  const loadUser = async () => {
    const res = await axios.get(`${window.base}/statmonitor/findById/${id}`);
    setUser(res.data);
  };
  return (
    <div className="container py-4">
      ...
      <h1 className="display-6">Stat Monitor Id: {id}</h1>
      <hr />
...
export default StatMonitorData;