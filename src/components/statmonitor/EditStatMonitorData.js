...
const EditStatMonitorData = () => {
  ...
  const onSubmit = async e => {
    e.preventDefault();
    await axios.put(`${window.base}/statmonitor/update/`, user);
    notify();
    history.push("/stat-monitor");
  };

  const loadUser = async () => {
    const result = await axios.get(`${window.base}/statmonitor/findById/${id}`);
    setUser(result.data);
  };
  return (
    <div className="container">
      ...
      <Link className="btn btn-outline-dark" to="/stat-monitor">
        ...
        <h2 className="text-center mb-4">Edit Stat Monitor</h2>
        <form onSubmit={e => onSubmit(e)}>
          ...
          <button className="btn btn-warning btn-block">Update Stat Monitor</button>
        </form>
      </div>
    </div>
  );
...
export default EditStatMonitorData;