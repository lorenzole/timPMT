...
const EditCodeStatMonitorData = () => {
  ...
  const onSubmit = async e => {
    e.preventDefault();
    await axios.put(`${window.base}/codestatmonitor/update/`, user);
    notify();
    history.push("/code-stat-monitor");
  };

  const loadUser = async () => {
    const result = await axios.get(`${window.base}/codestatmonitor/findById/${id}`);
    setUser(result.data);
  };
  return (
    <div className="container">
      ...
      <Link className="btn btn-outline-dark" to="/code-stat-monitor">
        ...
        <h2 className="text-center mb-4">Edit Code Stat Monitor</h2>
        <form onSubmit={e => onSubmit(e)}>
          ...
          <button className="btn btn-warning btn-block">Update Code Stat Monitor</button>
        </form>
      </div>
    </div>
  );
...
export default EditCodeStatMonitorData;