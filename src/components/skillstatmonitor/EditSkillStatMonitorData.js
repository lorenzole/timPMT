...
const EditSkillStatMonitorData = () => {
  toast.configure();
  const notify = () => {
  ...
  const onSubmit = async e => {
    e.preventDefault();
    await axios.put(`${window.base}/skillstat/update/`, dnMonitorData);
    notify();
    history.push("/skill-stat-monitor");
  };

  const loadUser = async () => {
    const result = await axios.get(`${window.base}/skillstat/findById/${id}`);
  ...
  return (
    <div className="container">
      ...
      <Link className="btn btn-outline-dark" to="/skill-stat-monitor">
        ...
        <h2 className="text-center mb-4">Edit Skill Stat</h2>
        <form onSubmit={e => onSubmit(e)}>
          ...
          <button className="btn btn-warning btn-block">Update Skill Stat Monitor</button>
        </form>
      </div>
    </div>
  );
...
export default EditSkillStatMonitorData;