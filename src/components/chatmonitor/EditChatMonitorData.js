...
const EditChatMonitorData = () => {
  ...
  const onSubmit = async e => {
    e.preventDefault();
    await axios.put(`${window.base}/chatmonitor/update/`, user);
    notify();
    history.push("/chat-monitor");
  };

  const loadUser = async () => {
    const result = await axios.get(`${window.base}/chatmonitor/findById/${id}`);
    setUser(result.data);
  };
  return (
    <div className="container">
      <div className="w-75 mx-auto shadow p-5">
      <Link className="btn btn-outline-dark" to="/chat-monitor">
        Back to Chat Monitor
      </Link>
  <h2 className="text-center mb-4">Edit Chat Monitor</h2>
...
export default EditChatMonitorData;