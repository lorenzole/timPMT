...
const AddDnMonitorData = () => {
  ...
  const onSubmit = async e => {
    e.preventDefault();
    await axios.post(`${window.base}/dnmonitor/insert`, dnMonitorData)
    .then(result => {
      history.replace("/dn-monitor");
      notify();
    })
    .catch(err => {
      errorNotify();
  ...
  return (
    ...
    <Link className="btn btn-outline-dark" to="/dn-monitor">
      ...
      <h2 className="text-center mb-4">Add DN Monitor Configuration</h2>
...
export default AddDnMonitorData;