...
const CodeStatMonitor = () => {
  ...
  const deleteProduct = () => {
    ...
    /* console.log(ids); */
     axios.delete(`${window.base}/codestatmonitor/delete`, {data: ids}, {
         headers: {
  ...
  useEffect(() =>{
   ...
   const loadUsers = async () => {
     try{
       const result = await axios.get(`${window.base}/codestatmonitor/findAll`);
   ...
   const deleteUser = async id => {
      await axios.delete(`${window.base}/codestatmonitor/delete/${id}`)
      notify();
      loadUsers();
  }
 const actionBodyTemplate = (rowData) => {
      ...
      return (
          <React.Fragment>
               <Link className="btn btn-primary mr-2" to={`/code-stat-monitor/${rowData.id}`}><FontAwesomeIcon icon={faEye} /></Link>
               <Link className="btn btn-outline-primary mr-2" to={`/code-stat-monitor/edit/${rowData.id}`}><FontAwesomeIcon icon={faPen} /></Link>
  ...
  const leftToolbarTemplate = () => {
      return (
          ...
          <h5 className="p-m-0">Manage Code Stat Monitor</h5>
          
              <Link icon="pi plus-circle" className="btn btn-outline-dark pb-2 mr-4" to='/code-stat-monitor/add'>
  ...
  return (
      ...
      <h1 className="display-4 mt-4">Code Stat Monitor</h1>
    </div>
...
export default CodeStatMonitor;