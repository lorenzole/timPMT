...
const ChatMonitor = () => {
  ...
  const deleteProduct = () => {
    ...
    /* console.log(ids); */
     axios.delete(`${window.base}/chatmonitor/delete`, {data: ids}, {
         headers: {
  ...
  useEffect(() =>{
   ...
   const loadUsers = async () => {
     try{
       const result = await axios.get(`${window.base}/chatmonitor/findAll`);
   ...
   const deleteUser = async id => {
      await axios.delete(`${window.base}/chatmonitor/delete/${id}`)
      notify();
      loadUsers();
  }
 const actionBodyTemplate = (rowData) => {
      ...
      return (
          <React.Fragment>
               <Link className="btn btn-primary mr-2" to={`/chat-monitor/${rowData.id}`}><FontAwesomeIcon icon={faEye} /></Link>
               <Link className="btn btn-outline-primary mr-2" to={`/chat-monitor/edit/${rowData.id}`}><FontAwesomeIcon icon={faPen} /></Link>
  ...
  const leftToolbarTemplate = () => {
      return (
          ...
          <h5 className="p-m-0">Manage Records</h5>
              ...
              <Link icon="pi plus-circle" className="btn btn-outline-dark pb-2 mr-4" to='/chat-monitor/add'>
  ...
  return (
      ...
      <h1 className="display-4 mt-4">Chat Monitor</h1>
    </div>
