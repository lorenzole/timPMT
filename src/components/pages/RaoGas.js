...
const RaoGas = () => {
  ...
  const deleteProduct = () => {
    ...
    /* console.log(ids); */
     axios.delete(`${window.base}/raogas/delete`, {data: ids}, {
         headers: {
  ...
  useEffect(() =>{
   ...
   const loadUsers = async () => {
     try{
       const result = await axios.get(`${window.base}/raogas/findAll`);
   ...
   const deleteUser = async id => {
      await axios.delete(`${window.base}/raogas/delete/${id}`)
      notify();
      loadUsers();
  }
 const actionBodyTemplate = (rowData) => {
      ...
      return (
          <React.Fragment>
               <Link className="btn btn-primary mr-2" to={`/rao-gas/${rowData.id}`}><FontAwesomeIcon icon={faEye} /></Link>
               <Link className="btn btn-outline-primary mr-2" to={`/rao-gas/edit/${rowData.id}`}><FontAwesomeIcon icon={faPen} /></Link>
  ...
  const leftToolbarTemplate = () => {
      return (
          ...
          <h5 className="ml-0">Manage Records</h5>
              ...
              <Link icon="pi plus-circle" className="btn btn-outline-dark mr-4" to='/rao-gas/add'>
  ...
  return (
      ...
      <h1 className="display-4 mt-4">Rao Gas</h1>
    </div>
