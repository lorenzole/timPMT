...
const SkillStatMonitor = () => {
  ...
  const deleteProduct = () => {
    ...
    /* console.log(ids); */
     axios.delete(`${window.base}/skillstat/delete`, {data: ids}, {
         headers: {
  ...
  useEffect(() =>{
   ...
   const loadUsers = async () => {
     try{
       const result = await axios.get(`${window.base}/skillstat/findAll`);
   ...
   const deleteUser = async id => {
      await axios.delete(`${window.base}/skillstat/delete/${id}`)
      notify();
      loadUsers();
  }
 const actionBodyTemplate = (rowData) => {
      ...
      return (
          <React.Fragment>
               <Link className="btn btn-primary mr-2" to={`/skill-stat-monitor/${rowData.id}`}><FontAwesomeIcon icon={faEye} /></Link>
               <Link className="btn btn-outline-primary mr-2" to={`/skill-stat-monitor/edit/${rowData.id}`}><FontAwesomeIcon icon={faPen} /></Link>
  ...
  const leftToolbarTemplate = () => {
      return (
          ...
          <h5 className="ml-0">Manage Records</h5>
          
              <Link icon="pi plus-circle" className="btn btn-outline-dark mr-4" to='/skill-stat-monitor/add'>
  ...
  return (
      ...
      <h1 className="display-4 mt-4">Skill Stat Monitor</h1>
    </div>
          ...
          <div className="card">
              <DataTable value={users} className="p-datatable-striped" paginator
                  ...
                  <Column field="tServerSipServer" header="Tserver/Sipserver" sortable filter filterPlaceholder="Search by tServerSipServer"></Column>
                  ...
                  <Column field="isRunning" header="IS RUNNING" sortable filter filterPlaceholder="Search by isRunning"></Column>
                  <Column field="ipServerTcf" header="ip server tcf" sortable filter filterPlaceholder="Search by ip server tcf"></Column>
...
export default SkillStatMonitor;