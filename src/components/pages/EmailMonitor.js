...
const EmailMonitor = () => {
  ...
  const deleteProduct = () => {
    ...
    /* console.log(ids); */
     axios.delete(`${window.base}/emailmonitor/delete`, {data: ids}, {
         headers: {
  ...
  useEffect(() =>{
   ...
   const loadUsers = async () => {
     try{
       const result = await axios.get(`${window.base}/emailmonitor/findAll`);
   ...
   const deleteUser = async id => {
      await axios.delete(`${window.base}/emailmonitor/delete/${id}`)
      notify();
      loadUsers();
  }
 const actionBodyTemplate = (rowData) => {
      ...
      return (
          <React.Fragment>
               <Link className="btn btn-primary mr-2" to={`/email-monitor/${rowData.id}`}><FontAwesomeIcon icon={faEye} /></Link>
               <Link className="btn btn-outline-primary mr-2" to={`/email-monitor/edit/${rowData.id}`}><FontAwesomeIcon icon={faPen} /></Link>
  ...
  const leftToolbarTemplate = () => {
      return (
          ...
          <h5 className="p-m-0">Manage Records</h5>
              ...
              <Link icon="pi plus-circle" className="btn btn-outline-dark pb-2 mr-4" to='/email-monitor/add'>
  ...
  return (
      ...
      <h1 className="display-4 mt-4">Email Monitor</h1>
    </div>
