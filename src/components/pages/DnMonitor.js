...
const DnMonitor = () => {
  ...
  const deleteProduct = () => {
    ...
    /* console.log(ids); */
     axios.delete(`${window.base}/dnmonitor/delete`, {data: ids}, {
         headers: {
  ...
  useEffect(() =>{
   ...
   const loadUsers = async () => {
     try{
       const result = await axios.get(`${window.base}/dnmonitor/findAll`);
   ...
   const deleteUser = async id => {
      await axios.delete(`${window.base}/dnmonitor/delete/${id}`)
      notify();
      loadUsers();
  }
 ...
 const actionBodyTemplate = (rowData) => {
      ...
      return (
          <React.Fragment>
               <Link className="btn btn-primary mr-2" to={`/dn-monitor/${rowData.id}`}><FontAwesomeIcon icon={faEye} /></Link>
               <Link className="btn btn-outline-primary mr-2" to={`/dn-monitor/edit/${rowData.id}`}><FontAwesomeIcon icon={faPen} /></Link>
               <Link className="btn btn-danger mr-2" onClick={() => deleteUser(rowData.id)}><FontAwesomeIcon icon={faTrash}/></Link>
          </React.Fragment>
      )
  }
  /* ICONE A FINE TABELLA REFRESHE E CLOUD */
  ...
  const leftToolbarTemplate = () => {
      return (
          ...
          <h5 className="ml-0">Manage Records</h5>
              <Link icon="pi plus-circle" className="btn btn-outline-dark mr-4" to='/dn-monitor/add'>
  ...
  return (
      ...
      <h1 className="display-4 mt-4">DN Monitor</h1>
    </div>
