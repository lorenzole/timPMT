...
const StatMonitor = () => {
  ...
  const deleteProduct = () => {
    ...
    /* console.log(ids); */
     axios.delete(`${window.base}/statmonitor/delete`, {data: ids}, {
         headers: {
  ...
  useEffect(() =>{
   ...
   const loadUsers = async () => {
     try{
       const result = await axios.get(`${window.base}/statmonitor/findAll`);
   ...
   const deleteUser = async id => {
      await axios.delete(`${window.base}/statmonitor/delete/${id}`)
      notify();
      loadUsers();
  }
 const actionBodyTemplate = (rowData) => {
      ...
      return (
          <React.Fragment>
               <Link className="btn btn-primary mr-2" to={`/stat-monitor/${rowData.id}`}><FontAwesomeIcon icon={faEye} /></Link>
               <Link className="btn btn-outline-primary mr-2" to={`/stat-monitor/edit/${rowData.id}`}><FontAwesomeIcon icon={faPen} /></Link>
  ...
  const leftToolbarTemplate = () => {
      return (
          ...
          <h5 className="p-m-0">Manage State Monitor</h5>
          
              <Link icon="pi plus-circle" className="btn btn-outline-dark pb-2 mr-4" to='/stat-monitor/add'>
  ...
  return (
      ...
      <h1 className="display-4 mt-4">Stat Monitor</h1>
    </div>
