...
const EditQueueMonitorData = () => {
  ...
  const onSubmit = async e => {
    e.preventDefault();
    await axios.put(`${window.base}/monitorqueue/update/`, user);
    notify();
    history.push("/queue-monitor");
  };

  const loadUser = async () => {
    const result = await axios.get(`${window.base}/monitorqueue/findById/${id}`);
    setUser(result.data);
  };
  return (
...
export default EditQueueMonitorData;